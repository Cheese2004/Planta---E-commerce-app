...
const ProductByCateComponent = props => {
    ...
    return (
      <ScrollView
        ...
        {data.map((item, index) => {
          return (
            <TouchableOpacity
              style={[{height: childHeight2}, styles.singItem]}
              key={index}
    ...
    <View style={styles.sectionContainer}>
      ...
      {!!title && (
        <TouchableOpacity onPress={}>
          <Text style={styles.linkMore}>Xemm thêm {title}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
export default ProductByCateComponent;
